...
def run_fastlmmc_specified_index(dataset, output_dir, process_id, group_size, covFile=None, species='mouse', maxthreads=1, featsel=False, exclude=False, condition=None):
	...
	# index pair existed in the pheno file.
	csv = pd.read_csv(pheno)
	# first two columns
	csv = csv.iloc[:,:2]
	start = process_id*group_size ; end = min((process_id + 1)*(group_size), n + 1)
	re = pd.DataFrame()
	for i in range(start, end):
		df = epistasis(filtered_snp_reader, pheno, G0=full_snp_reader,
				   sid_list_0=filtered_snp_reader.sid[csv.iloc[i,0]],
				   sid_list_1=filtered_snp_reader.sid[csv.iloc[i,1]])
		re.append(pd)

	v.update(locals())
	# output to csv
	final = format_results(re, final_columns, p_value_threshold)
	final.to_csv('%(output_dir)s/%(dataset)s_%(process_id)s.gwas' % v, sep='\t', index=False)
